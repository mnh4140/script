...
fixResultLog2="${osname}_${osver}_Fix_$hostname.log"
...
function FixLog() {
    ...
    ((Num++))
    # 점검코드|점검항목명 로깅
    echo -e " $Num. [$1]" | tee -a "$logPATH/$fixResultLog1"
    echo -e " $Num. [$1]" >> "$logPATH/$fixResultLog2"
    ...
    if [ "$5" = "SAFE" ]; then
        ...
        echo -e "\t- Check result:\t$5" | tee -a "$logPATH/$fixResultLog1"
        echo -e "\t- Check result:\t$5" >> "$logPATH/$fixResultLog2"
    fi

    # 권장 설정값 및 현재 설정값 로깅
    echo -e "\t- Suggestion:\t$2" | tee -a "$logPATH/$fixResultLog1"
    echo -e "\t- Before:\t$3" | tee -a "$logPATH/$fixResultLog1"
    echo -e "\t- After:\t$4" | tee -a "$logPATH/$fixResultLog1"
    echo -e "\n" | tee -a "$logPATH/$fixResultLog1"

    echo -e "\t- Suggestion:\t$2" >> "$logPATH/$fixResultLog2"
    echo -e "\t- Befor:\t$3" >> "$logPATH/$fixResultLog2"
    echo -e "\t- After:\t$4" >> "$logPATH/$fixResultLog2"
    echo -e "\n" >> "$logPATH/$fixResultLog2"
}
