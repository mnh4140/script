...
function U-10() {
	...
	if [ -e $CONFIG_FILE ]; then # 설정 파일이 존재
		...
		securityState="${GREEN}SAFE${RESET}:설정파일 없음"
		permission=""
                owner=""
	fi
	...
	if [ -z "$securityState" ]; then # $securityState 값 없으면
        ...
        securityLog "${FUNCNAME[0]}|(x)inetd.conf  파일 소유자 및 권한 설정" "$suggestionValue" "$currentValue" "$securityState"
