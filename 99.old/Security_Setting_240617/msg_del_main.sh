...
# main.sh 에서 취약점 점검 시 dialog msg 누적 방지 기능 추가 버번
...
function SshRestart()	#	SSH 데몬 재시작 함수
{
    ...
    case $answer in 
        0)
			...
			echo "# SSHD Service restart completed." >> $LOGFILE
			echo "" >> $LOGFILE
        ;;
        1)
			echo "# Didn't restart the SSHD Service." >> $LOGFILE
			echo "" >> $LOGFILE
        ;;
        255)
			echo "# Didn't restart the SSHD Service." >> $LOGFILE
